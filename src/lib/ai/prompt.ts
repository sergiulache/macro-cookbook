...
- category: pick the SINGLE best fit from exactly this list - Breakfast, Lunch, Dinner, Snacks, Desserts, Ice Cream, Sorbets, Cookie Dough, Sauces, Breads, Meal Prep (use Dinner for a generic savory main dish). prepTimeMin and cookTimeMin: realistic whole-minute estimates of hands-on prep time and total cooking/baking/chilling time for this recipe (use 0 for cook time if nothing is cooked).
...
- Produce TWO languages. The English fields (title, name, steps) are clear English. The Romanian fields (title_ro, name_ro, steps_ro) are a natural Romanian translation - not word-for-word - and MUST use correct Romanian diacritics (ă, â, î, ș, ț) everywhere. LOCALIZE American ingredients to the Romanian product a shopper in Romania would buy: e.g. graham crackers -> biscuiți digestivi; heavy cream -> smântână pentru frișcă; all-purpose flour -> făină albă tip 000; cilantro -> coriandru; scallions -> ceapă verde; sour cream -> smântână; cornstarch -> amidon de porumb; baking soda -> bicarbonat de sodiu; powdered sugar -> zahăr pudră. name_ro MUST line up 1:1 with the English ingredients (same items, same order), and steps_ro MUST have the same number of steps as steps, in the same order. Quantities/macros are identical across languages.
