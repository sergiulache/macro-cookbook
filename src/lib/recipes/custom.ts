...
export function customToRecipe(c: CustomRecipe, lang: "en" | "ro" = "en"): Recipe {
  const ro = lang === "ro";
  const ingredients = c.lines.map((l) => ({ amount: l.grams, unit: "g", item: (ro && l.name_ro) || l.name, optional: l.optional }));
...
/** A custom recipe has a Romanian version when the AI produced one. */
export const hasRomanian = (c: CustomRecipe) => !!(c.title_ro || c.steps_ro?.length || c.lines.some((l) => l.name_ro && l.name_ro !== l.name));
